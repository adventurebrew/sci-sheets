...
const defaultFileMode = constants.TPERMALL;  // rwxrwxrwx
...
const posixHeader = [
  ...
  ['mode', 8, 100, function(file, field) {
    let mode = file[field[0]] || defaultFileMode;
...
const effectiveHeaderSize = (function (header) {
...
function parseTarString(bytes, returnUnprocessed) {
  ...
  const index = result.indexOf(constants.NULL_CHAR);
...
function calculateChecksum(buffer, offset, skipChecksum) {
  ...
  const to = Math.min(from + effectiveHeaderSize, buffer.length);
  let result = 0;
  ...
  for (i = from; i < to; i++) {
    ...
    const byte = (i >= skipFrom) && (i < skipTo) ? whitespace : buffer[i];
