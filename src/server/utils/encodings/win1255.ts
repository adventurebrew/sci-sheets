...
const WIN1255_TO_UNICODE_RAW: Array<[number, number]> = [
  [0x00, 0x0000],    // NULL
...
function inverseKeyValue<K, V>([k, v]: [K, V]): [V, K] {
  return [v, k];
}

const WIN1255_TO_UNICODE = new Map(WIN1255_TO_UNICODE_RAW)
const UNICODE_TO_WIN1255 = new Map(WIN1255_TO_UNICODE_RAW.map(inverseKeyValue))

export default {
  from: (code) => WIN1255_TO_UNICODE.has(code) ? WIN1255_TO_UNICODE.get(code) : UNKNOWN,
  to: (code) => UNICODE_TO_WIN1255.has(code) ? UNICODE_TO_WIN1255.get(code) : UNKNOWN,
}
